...
import org.climasense.databinding.ActivityDetailDisplayManageBinding
import org.climasense.settings.SettingsManager
import org.climasense.settings.adapters.DetailDisplayAdapter
import org.climasense.theme.ThemeManager
import java.util.*

class DetailDisplayManageActivity : GeoActivity() {
    private lateinit var mBinding: ActivityDetailDisplayManageBinding
    private lateinit var mDetailDisplayAdapter: DetailDisplayAdapter
    private var mDetailDisplayItemTouchHelper: ItemTouchHelper? = null
    ...
    private inner class DetailDisplaySwipeCallback : SlidingItemTouchCallback() {
        ...
        override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
            mDetailDisplayAdapter.removeItem(viewHolder.bindingAdapterPosition)
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding = ActivityDetailDisplayManageBinding.inflate(layoutInflater)
        ...
        val displayTags = SettingsManager.getInstance(this).detailDisplayList
        mDetailDisplayAdapter = DetailDisplayAdapter(
            displayTags.toMutableList(),
            { detailDisplay: DetailDisplay ->
                setResult(RESULT_OK)
                mTagAdapter!!.insertItem(DetailTag(detailDisplay))
                resetBottomBarVisibility()
            },
            { holder: DetailDisplayAdapter.ViewHolder ->
                mDetailDisplayItemTouchHelper!!.startDrag(holder)
            }
        )
        ...
        mBinding.recyclerView.adapter = mDetailDisplayAdapter
        ...
        for (detail in otherTags) {
            tagList.add(DetailTag(detail))
        }
    ...
    override fun onStop() {
        super.onStop()
        val oldList = SettingsManager.getInstance(this).detailDisplayList
        val newList = mDetailDisplayAdapter.detailDisplayList
        if (oldList != newList) {
            SettingsManager.getInstance(this).detailDisplayList = newList
        }
    }
