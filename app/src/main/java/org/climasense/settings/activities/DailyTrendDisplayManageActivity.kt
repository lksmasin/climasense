...
import org.climasense.databinding.ActivityDailyTrendDisplayManageBinding
import org.climasense.settings.SettingsManager
import org.climasense.settings.adapters.DailyTrendDisplayAdapter
...
class DailyTrendDisplayManageActivity : GeoActivity() {
    ...
    private inner class DailyTrendTag(
        var tag: DailyTrendDisplay
    ) : TagAdapter.Tag {
        override val name = tag.getName(this@DailyTrendDisplayManageActivity)
    }
    ...
    private inner class CardDisplaySwipeCallback : SlidingItemTouchCallback() {
        ...
        override fun onSwiped(viewHolder: RecyclerView.ViewHolder, direction: Int) {
            mDailyTrendDisplayAdapter.removeItem(viewHolder.bindingAdapterPosition)
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mDailyTrendDisplayAdapter = DailyTrendDisplayAdapter(
            ...
            { dailyTrendDisplay: DailyTrendDisplay ->
                setResult(RESULT_OK)
                mTagAdapter!!.insertItem(DailyTrendTag(dailyTrendDisplay))
                resetBottomBarVisibility()
            },
            { holder: DailyTrendDisplayAdapter.ViewHolder ->
                mDailyTrendDisplayItemTouchHelper!!.startDrag(holder)
            }
        )
        ...
        mBinding.recyclerView.adapter = mDailyTrendDisplayAdapter
        ...
        mTagAdapter = TagAdapter(
            ...
            { _: Boolean, _: Int, newPosition: Int ->
                setResult(RESULT_OK)
                val tag = mTagAdapter!!.removeItem(newPosition) as DailyTrendTag
    ...
    override fun onStop() {
        super.onStop()
        val oldList = SettingsManager.getInstance(this).dailyTrendDisplayList
        val newList = mDailyTrendDisplayAdapter.dailyTrendDisplayList
        if (oldList != newList) {
            SettingsManager.getInstance(this).dailyTrendDisplayList = newList
        }
    }
